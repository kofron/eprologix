...
-record(request,{from,data,tag}).

%% API
-export([start_link/0]).
-export([configuring/2,waiting/2,waiting/3,sending/2,receiving/2,parse_reply/2]).
-export([send_query/1]).
...
send_query(QueryString) ->
	Req = #request{from = self(), data = QueryString, tag = make_ref()},
...
sending(timeout, #state{c_req=R,c_sock=S}=StateData) ->
	...
	{next_state, receiving, StateData,0}.
...
parse_reply(timeout, #state{c_sock=S}=StateData) ->
	close_telnet_sock(S),
